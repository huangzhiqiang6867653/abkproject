...
public class JdbcRealmTest {
    ...
    public void test() {
        ...
        Factory<SecurityManager> factory = new IniSecurityManagerFactory("classpath:config/jdbc_realm.ini");
        ...
        try {
            subject.login(token);
            // 6.判断是否成功登录
            assertEquals("登录校验：",true, subject.isAuthenticated());
